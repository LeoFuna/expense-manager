...
export default class TransactionController implements ITransactionController {
  ...
  async getTransactionBalance(request: any, { params }: { params: { email: string; month: number; }; }): Promise<NextResponse<{ totalIncomeInCents: number; totalOutcomeInCents: number; }>> {
    const transactions = await this.transactionService.getTransactionBalance(params.email, params.month);
